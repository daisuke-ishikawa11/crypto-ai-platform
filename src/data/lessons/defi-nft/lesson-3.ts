...
export const lesson3: Lesson = {
  ...
  content: {
    sections: [
      {
        type: 'text',
        content: `# 流動性提供とイールドファーミング

**流動性提供（Liquidity Provision）**は、DEXの取引ペアに資金を提供することで**取引手数料収入を獲得**する行為です。**イールドファーミング（Yield Farming）**は、流動性提供により得られたLPトークンを更に他のプロトコルに預けて**追加収益を獲得**する高度な戦略です。
...
- 新機能・プロトコルの早期参加`
      ...
      {
        type: 'example',
        content: `## 実践例：段階的イールドファーミング戦略

### Phase 1: 基礎戦略（初期資金：10,000ドル）

**Month 1-2: 安定収益確立**
- **Curve 3Pool**: 5,000ドル（USDC/USDT/DAI）
  - 期待年利: 8%
  - リスク: 極低（ステーブルコインのみ）
  - 月間収益: 約33ドル

- **Uniswap V3 USDC/ETH**: 3,000ドル
  - 価格レンジ: ±5%（狭いレンジで高効率）
  - 期待年利: 25%
  - 非常時損失リスク: 中程度
  - 月間収益: 約62ドル

- **残り現金**: 2,000ドル（機会待ち・ガス代等）
...
2. **情報収集の重要性**:
   - DefiPulse、APY.vision等での収益率比較
   - Twitter、Discord等でのコミュニティ情報
   - 監査レポート・プロトコル分析

3. **税務・記録管理**: 複雑な取引の詳細記録と適切な税務処理が長期成功の基盤！`
      },
...
**最重要**: イールドファーミングは高収益の可能性がありますが、複数の新しいリスクが存在します。十分な理解と慎重なリスク管理が成功の鍵です。`
      }
    ],

    keyPoints: [
      '流動性提供は取引手数料収入を得る仕組みで、イールドファーミングは追加収益獲得の高度戦略',
      '非常時損失は価格変動による機会損失で、価格2倍で約5.7%の損失が発生',
      'ステーブルコインペアは低リスク、価格相関の高いペア選択が重要',
      'レバレッジ・デルタニュートラル等の高度戦略でリスクとリターンを調整可能',
      'Yearn・Convex等の最適化プロトコルで効率的な収益獲得が可能',
      '段階的なアプローチでリスクを管理しながら戦略を拡張',
      '税務処理・記録管理が複雑で、専門ツールや専門家活用が推奨',
      '継続的な学習・情報収集・戦略改善が長期的成功の基盤'
    ]
  }
}; 